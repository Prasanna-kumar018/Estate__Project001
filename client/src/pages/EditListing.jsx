...
const EditListing = () => {
  ...
  let { edit_list_id } = useParams();
  React.useEffect(() => {
    setfullloading(true);
    setfullerror("");
    console.log(edit_list_id);
    async function abc() {
      let d = await fetch(`/api/listing/listings/get/${edit_list_id}`);
  ...
  async function handleSubmit(e) {
    ...
    if (fileUrls.length == 0) {
      seterror("Atleast One image Required....");
      return;
    }
    ...
    try {
      ...
      let result = await fetch(`/api/listing/listings/update/${data._id}`, {
        method: "PUT",
        headers: {
  ...
  function handleChange(e) {
    if (e.target.id === "description" || e.target.type === "text") {
      ...
      setData({ ...data, [e.target.id]: parseInt(e.target.value) });
    } else if (
  ...
  if (fullloading) {
    ...
    return (
      <div className="w-full md:mx-28 max-md:mx-10 mb-10">
        <h1 className="text-center font-bold text-3xl my-5">Update Listing</h1>
